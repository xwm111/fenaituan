...
public class Product extends BaseEntity {
	...
	// 托管（未托管、已申请托管、已托管、已申请取消托管）
	public enum Trusteeship {
		unused, processing, received, cancel
	}
	...
	public static final String SALES_TYPE_SECONDARY = "secondary"; // 销售类型
	...
	private String salesType; // 商品销售类型
	private Trusteeship trusteeship; // 托管
	...
	public void setMarketPrice(BigDecimal marketPrice) {
		this.marketPrice = SystemConfigUtil
				.getPriceScaleBigDecimal(marketPrice);
	}
	...
	public List<ProductImage> getProductImageList() {
		...
		return (List<ProductImage>) JSONSerializer
	...
	public Map<ProductAttribute, List<String>> getProductAttributeMap() {
		if (productAttributeMapStore == null
				|| productAttributeMapStore.size() == 0) {
			return null;
		}
		Map<ProductAttribute, List<String>> productAttributeMap = new HashMap<ProductAttribute, List<String>>();
		for (ProductAttribute productAttribute : productAttributeMapStore
				.keySet()) {
			String productAttributeValueStore = productAttributeMapStore
					.get(productAttribute);
			if (StringUtils.isNotEmpty(productAttributeValueStore)) {
				JSONArray jsonArray = JSONArray
						.fromObject(productAttributeMapStore
								.get(productAttribute));
				productAttributeMap.put(productAttribute,
						(List<String>) JSONSerializer.toJava(jsonArray));
			} else {
	...
	public void setProductAttributeMap(
			Map<ProductAttribute, List<String>> productAttributeMap) {
		...
		for (ProductAttribute productAttribute : productAttributeMap.keySet()) {
			List<String> productAttributeValueList = productAttributeMap
					.get(productAttribute);
			if (productAttributeValueList != null
					&& productAttributeValueList.size() > 0) {
				JSONArray jsonArray = JSONArray
						.fromObject(productAttributeValueList);
	...
	public BigDecimal getPreferentialPrice(Member member) {
		if (member != null) {
			BigDecimal preferentialPrice = price.multiply(new BigDecimal(member
					.getMemberRank().getPreferentialScale().toString())
	...
	public Trusteeship getTrusteeship() {
		return trusteeship==null?Trusteeship.unused:trusteeship;
	}
