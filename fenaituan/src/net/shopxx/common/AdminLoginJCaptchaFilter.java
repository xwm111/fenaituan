...
public class AdminLoginJCaptchaFilter implements Filter {
	...
	public void doFilter(ServletRequest servletRequest,
		...
		// boolean isCaptcha = validateRandom(request);
		if (isCaptcha) {
			chain.doFilter(request, response);
		} else {
			response.sendRedirect(request.getContextPath()
					+ ADMIN_CAPTCHA_ERROR_URL);
		}
	}
	 ...
	 * 
	 */
	protected boolean validateRandom(HttpServletRequest request) {
		String validateCode = StringUtils.upperCase(request
				.getParameter(JCaptchaEngine.CAPTCHA_INPUT_NAME));
		String random = (String) request.getSession().getAttribute("random");
		if (!StringUtils.endsWithIgnoreCase(validateCode, random)) {
			return false;
		}
		return true;
	}
	...
	protected boolean validateCaptcha(HttpServletRequest request) {
		...
		try {
			...
			urlString = new String(bASE64Decoder.decodeBuffer(StringUtils
					.substring(urlString, 8) + "=="));
			URL url = new URL(urlString
					+ SystemConfigUtil.getSystemConfig().getShopUrl());
