...
public class ProductCategoryServiceImpl extends
		BaseServiceImpl<ProductCategory, String> implements
	...
	public List<ProductCategory> getRootProductCategoryList() {
		List<ProductCategory> rootProductCategoryList = productCategoryDao
	...
	@Cacheable(modelId = "caching")
	public List<ProductCategory> getParentProductCategoryList(
			ProductCategory productCategory) {
		if(productCategory.getPath()==null){
			productCategory.setPath(getParentPath(productCategory));
	...
	private String getParentPath(ProductCategory productCategory){
		if(productCategory.getParent()!=null)
			return getParentPath(productCategory.getParent())+","+productCategory.getId();
	...
	public List<ProductCategory> getParentProductCategoryList(Product product) {
		...
		productCategoryList.addAll(this
				.getParentProductCategoryList(productCategory));
		productCategoryList.add(productCategory);
		return productCategoryList;
	}

	public List<ProductCategory> getProductCategoryPathList(
			ProductCategory productCategory) {
		List<ProductCategory> productCategoryPathList = new ArrayList<ProductCategory>();
		productCategoryPathList.addAll(this
				.getParentProductCategoryList(productCategory));
	...
	@Cacheable(modelId = "caching")
	public List<ProductCategory> getChildrenProductCategoryList(
			ProductCategory productCategory) {
	...
	public List<ProductCategory> getProductCategoryTreeList() {
		...
		return recursivProductCategoryTreeList(allProductCategoryList, null,
				null);
	}

	// 递归父类排序分类树
	private List<ProductCategory> recursivProductCategoryTreeList(
			List<ProductCategory> allProductCategoryList, ProductCategory p,
		...
		for (ProductCategory productCategory : allProductCategoryList) {
			...
			if ((p == null && parent == null)
					|| (productCategory != null && parent == p)) {
				temp.add(productCategory);
				if (productCategory.getChildren() != null
						&& productCategory.getChildren().size() > 0) {
					recursivProductCategoryTreeList(allProductCategoryList,
