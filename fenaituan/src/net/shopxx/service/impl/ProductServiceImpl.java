...
public class ProductServiceImpl extends BaseServiceImpl<Product, String>
	...
	public Pager getProductPager(ProductCategory productCategory, Pager pager,
			Region region) {
		return productDao.getProductPager(productCategory, pager, regionService
				.getAllChildRegionByRegionId(region == null ? null : region
						.getId()));
	}
		...
		if (admin != null) {
			if (region != null) {
				List<Region> list = regionService
						.getAllChildRegionByRegionId(region.getId());
				for (Region region2 : list) {
					if (admin.getRegionSet().contains(region2))
						regions.add(region2);
				}
			} else
				regions.addAll(admin.getRegionSet()); // 为空则搜索该管理员所管理的所有区域中的数据
		} else {
			if (region != null) {
				regions.addAll(regionService.getAllChildRegionByRegionId(region
						.getId()));
			}
		}
	...
	@Override
	public Pager getSecondaryProductPagerByMember(ProductCondition condition,
			Member member) {
		if (member == null)
			return new Pager();
		if (condition == null)
		...
		condition.setSalesType(Product.SALES_TYPE_SECONDARY);
		return productDao.queryForPager(condition);
	}
