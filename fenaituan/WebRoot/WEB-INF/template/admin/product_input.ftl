		...
		<form id="inputForm" class="validate" action="<#if isAdd??>product!save.action<#else>product!update.action</#if>" method="post" enctype="multipart/form-data" >
			...
			<table class="inputTable tabContent">
				...
				<tr>
					...
					<td>
						<select name="product.trusteeship">
							<#list allTrusteeship as list>
								<option value="${list}"<#if (list == product.trusteeship)!> selected </#if>>
									${action.getText("Trusteeship." + list)}
								</option>
							</#list>
						</select>
					</td>
			...
			<input type="hidden" id="regionIds" name="regionIds" value="${regionIds}" />
				...
				<tr>
					<td>
						<div id="deptArea" style="float:left;" >
