...
import net.shopxx.util.VerificationCodeUtil;
...
@ParentPackage("admin")
@Results({ @Result(name = "random", type = "stream", params = { "contentType",
		"image/jpeg", "inputName", "inputStream", "bufferSize", "4096" }) })
public class AdminAction extends BaseAdminAction {
	...
	private String adminRegionData = "";
	...
	public String getRandomPictrue() {
		...
		ActionContext.getContext().getSession()
				.put("random", vcu.getVerificationCodeValue());// 取得随机字符串放入HttpSession
		return "random";
	...
	public String selectRegion() {
		String loginUsername = (String) ActionContext.getContext().getSession()
				.get("SPRING_SECURITY_LAST_USERNAME");
		if (loginUsername != null) {
			Admin admin = adminService.get("username",
					loginUsername.toLowerCase());
			if (admin != null) {
				setAdminRegionData(regionService.getArrayByAdminId(
	...
	public String edit() {
		...
		init();
		regionCur = regionService.getArrayByAdminId(id).toString();
		Set<Region> regionSet = admin.getRegionSet();
	...
	private void init() {
		regionAll = regionService.getArrayAll().toString();
	}
	...
	public void setAdminRegionData(String adminRegionData) {
		this.adminRegionData = adminRegionData;
	}

	public String getAdminRegionData() {
		return adminRegionData;
	}
