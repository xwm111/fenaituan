	...
	public String all() throws Exception {
		...
		if (buildContent.equalsIgnoreCase("article")) {
			...
			if (buildType.equalsIgnoreCase("all")) {
				articleList = articleService.getArticleList(firstResult,
						maxResults);
				...
				articleList = articleService.getArticleList(beginDate, endDate,
		...
		if (buildContent.equalsIgnoreCase("product")) {
			...
			if (buildType.equalsIgnoreCase("all")) {
				productList = productService.getProductList(firstResult,
						maxResults);
			} else if (buildType.equalsIgnoreCase("date")) {
				if (beginDate != null) {
					beginDate.setHours(0);
				...
				productList = productService.getProductList(beginDate, endDate,
						firstResult, maxResults);
			}
	...
	public String article() {
		...
		if (StringUtils.isEmpty(id)) {
			articleList = articleService
					.getArticleList(firstResult, maxResults);
		} else {
			ArticleCategory articleCategory = articleCategoryService.load(id);
			articleList = articleService.getArticleList(articleCategory,
					firstResult, maxResults);
		}
		if (articleList != null && articleList.size() > 0) {
			for (Article article : articleList) {
	...
	public String product() {
		...
		if (StringUtils.isEmpty(id)) {
			productList = productService
					.getProductList(firstResult, maxResults);
		} else {
			ProductCategory productCategory = productCategoryService.load(id);
			productList = productService.getProductList(productCategory,
					firstResult, maxResults);
		}
		if (productList != null && productList.size() > 0) {
			for (Product product : productList) {
