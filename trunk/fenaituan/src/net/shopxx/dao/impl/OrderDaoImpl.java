...
public class OrderDaoImpl extends BaseDaoImpl<Order, String> implements
	...
	@Override
	public Pager getSecondaryOrderPager(Member member, Pager pager) {
		if (pager == null) {
			pager = new Pager();
		}
		if (pager.getPageSize() == null) {
		...
		DetachedCriteria detachedCriteria = DetachedCriteria
				.forClass(Order.class);
		detachedCriteria.createAlias("orderItemSet", "orderItem");
		detachedCriteria.createAlias("orderItem.product", "product");
		detachedCriteria.add(Restrictions.eq("product.salesType", Product.SALES_TYPE_SECONDARY));
		detachedCriteria.add(Restrictions.eq("product.creatorMem", member));
		return super.findByPager(pager, detachedCriteria);
	}
	...
	public Long getUnprocessedOrderCount() {
		...
		return (Long) getSession().createQuery(hql)
				.setParameter(0, OrderStatus.unprocessed).uniqueResult();
	}
	...
	public Long getPaidUnshippedOrderCount() {
		...
		return (Long) getSession().createQuery(hql)
				.setParameter(0, PaymentStatus.paid)
				.setParameter(1, ShippingStatus.unshipped)
				.setParameter(2, OrderStatus.completed)
