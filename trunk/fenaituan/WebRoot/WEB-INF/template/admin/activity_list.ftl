		...
		<form id="listForm" action="activity!list.action" method="post">
			<div class="operateBar">
				...
				<select name="pager.property">
					...
					<option value="creatorMem.username" <#if pager.property == "creatorMem.username">selected="selected" </#if>>
						用户名
					</option>
				</select>
			...
			<table class="listTable">
				<tr>
					...
					<th>
						<span class="sort" name="name">主题</span>
					</th>
					<th>
						<span >创建者</span>
					</th>
				...
				<#list pager.list as list>
					<tr>
						...
						<td>
							<a href="${base}/shop/activity!detail.action?id=${list.id}" target="_blank">${list.name!"&nbsp;"}</a> 
						</td>
						<td>
						...
						<td>
							${list.createDate?string("yyyy-MM-dd HH:mm:ss")}
									...
									<#else>
							<td>
							${list.lastReplyMem.username}
						</td>
						<td>
							${list.lastReplyDate?string("yyyy-MM-dd HH:mm:ss")}
						</td>
									</#if>
						<td>
							<#if list.isTop==true>
						...
						<td>
							<a href="activity!replyList.action?id=${list.id}" title="查看回复">[查看回复]</a>
			...
			<#if (pager.list?size > 0)>
				<div class="pagerBar">
					<input type="button" class="topButton" url="activity!topActivity.action" value="置顶"  hidefocus="true" />
					<input type="button" class="topButton" url="activity!cancelTop.action" value="取消置顶"  hidefocus="true" />
