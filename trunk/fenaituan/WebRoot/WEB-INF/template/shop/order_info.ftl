						...
						<td>
							<table class="paymentConfigTable">
								<#list allPaymentConfig as list>
									<#if (list.id == '402881862b9f9e78012b9fa02aca0002')!>
									<tr>
										<th>
											<label class="{paymentFeeType: '${list.paymentFeeType}', paymentFee: '${list.paymentFee}'}">
												<input type="radio" name="paymentConfig.id" class="{required: true, messages: {required: '请选择支付方式！'}}" checked=checked value="${list.id}" />
												${list.name}
											</label>
										</th>
										<td>
